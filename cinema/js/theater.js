...
function getPlayerByType( type ) {
	console.log('getting video player for ' + type)
...
(function() {
	...
	// made by @cuddlycheetah github.com/cuddlycheetah
	var DirectVideoByCuddlyCheetah = function() {
		var self = this;
		//self.lastVolume = 0.5
		this.setVideo = function( url ) {
			this.video = url;
			console.log('loaded CCDirectVideo Player for ' + url)
		...
		*/
		this.think = function() {
			...
			if ( this.player !== null ) {
				...
				self.currentTime = this.player.currentTime;
		...
		this.getBufferedTime = function() {
			return this.player.buffered.length;
		};
	...
	registerPlayer( "ccdirectvideo", DirectVideoByCuddlyCheetah );
